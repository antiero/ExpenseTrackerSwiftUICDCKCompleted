...
extension ExpenseLog {
    ...
    var categoryText: String {
        categoryEnum.rawValue.capitalized
    }
    ...
    var relativeDateText: String {
        Utils.relativeDateFormatter.localizedString(for: date ?? Date(), relativeTo: Date())
    }
    ...
    var shareContent: String {
        "Expense for \(nameText)(\(categoryText)) at \(dateText) with amount of \(amountText)"
    }
    
    var csvString: String {
        "\(nameText) ,\(dateText) , \(categoryText) , \(amountText)"
    }
